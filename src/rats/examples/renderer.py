...
class Renderer:
    def __init__(self, hide_window=False):
        ...
        self._render_thread = Thread(target=self._loop, name=(
            'render cameras'))
    ...
    def add_cameras(self, cameras: list[dict]):
    ...
    def add_aruco_marker(self, id, lat, lon, alt, yaw):
        m = MarkerObject(id, 0.20)
        m.set_position(np.array([lon, alt, -lat]))
        m.set_rotation(np.array([0, -yaw, 0]))
        self._marker_objects.append(m)
    ...
    def _loop(self):
        while self._running:
            self.render_and_process_window()

    def render_and_process_window(self):
        glutMainLoopEvent()
        return self._draw()
    ...
    def _draw(self):
        ...
        device = self._rendered_frame_keys[self._camera_index]
        # if self._frame_callback is not None and callable(self._frame_callback):
        #     self._frame_callback(device, width, height, frame_data)
        ...
        return device, self._convert_to_np_array(width, height, frame_data)
        # time.sleep(.01)
            
    def _convert_to_np_array(self, width, height, data):
        arr = np.frombuffer(data, dtype=np.uint8).reshape(height, width, 4)[:,:,:3]
        arr = np.flip(arr, 0)
...
def main():
    ...
    renderer.add_aruco_marker(0, 0.0, 0, 0, 0)
    # renderer.start()
    while True:
        renderer.render_and_process_window()
        # time.sleep(1)
